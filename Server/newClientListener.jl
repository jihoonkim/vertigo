function newClientListener(TIMEOUT, MSG_CODEBOOK, NETWORK, DB, ParaInServer)
  ...
  println("Server socket established at ",now());

  result=Dict{Any,Any}("clientCount"=>falses(size(DB["users"],1)),
                       "iter"=>ParaInServer["iter"],
                       ...
                       "kernel"=>cell(0),
                       "incoming_mw_old"=>cell(size(DB["users"],1)),
                       "posterior_new_mw"=>cell(size(DB["users"],1)));

  remotecall(2,initSocks,DB);
  println("Waiting for client's connection...");
  h = now()
  msgminus1 = 1;
  while !closeServerFlag
#    println("Still in while block:",!closeServerFlag)
    result, msg, msgout = remotecall_fetch(2,handleIncomingMessageFromClient,result,MSG_CODEBOOK,TIMEOUT,DB)
    socks_list, closeServerFlag, DB, result, msgout = checkMessageFromsocks_list(closeServerFlag, socks_list, TIMEOUT, MSG_CODEBOOK, DB, msg, msgout, msgminus1, result);
    DB, msgminus1 = asyncCheckMessageFromDataListener(msgout, socks_list, TIMEOUT, MSG_CODEBOOK, DB);
    if msgminus1 == -1;
...
function checkMessageFromsocks_list(closeServerFlag, socks_list, TIMEOUT, MSG_CODEBOOK, DB, msg, msgout,msgminus1, result)
  msg_list = cell(length(socks_list["socks"]));
  for i = 1:length(socks_list["socks"])
#    println("msg from client ", i, " is ", socks_list["isConfirmed"][i]);
...
#      println("Finish deserializing!")
  ...
  if !isempty(msg)
    if msg["tag"]==MSG_CODEBOOK["updateCalculation"]
      if !result["jobDone"]
        ...
        result, msgout= remotecall_fetch(2, updateAllSites, msg, msgminus1, result, MSG_CODEBOOK, TIMEOUT, DB);
      else
        println("Calculation is paused, sicne ADMMDPCA has converged");
      end
  ...
  if DB["joined_count"] == size(DB["jobs"], 1) && DB["joined_count_data"] == size(DB["jobs"], 1)
    if !DB["isActive"]
      ...
      if !result["jobDone"]
        println("Update calculation at iteration: ", result["iter"]+1);
        #call 2 to do updateAllSites
  ...
  return socks_list, closeServerFlag, DB, result, msgout
end
...
function asyncCheckMessageFromDataListener(msg, socks_list, TIMEOUT, MSG_CODEBOOK, DB)
  msgminus1=1;
  if !isempty(msg)
    ...
    elseif msg["tag"] == MSG_CODEBOOK["updateCalculation_Done"]
      println("Finish update at iter : ", msg["iter"]);
      msgminus1 = -1;
...
function closeRemoteSocket(sock,  MSG_CODEBOOK)
  ...
  if res == -1
    println("fail to send shutdown signal to clients");
  end
  close(sock);
